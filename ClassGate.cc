 ...
 *   the top of ClassGate.h, you will find a list of macros that give names to each gate type.
...
void Gate::setValue(int val) { 
	switch(val) {
		...
		case LOGIC_ONE: break;
		case LOGIC_UNSET: break;
		case LOGIC_X: break;
		case LOGIC_D: break;
		case LOGIC_DBAR: break;
		default: { 
			 cout << "ERROR: Trying to set gate " << get_outputName() << " to illegal value " << (char)val << ", " << val << endl; 
			 assert(false);
		}
	} 
	gateValue = val; 
...
/** \brief Set this gate's depth. 
 *  \param d Depth
 */
void Gate::setDepth(int d) {
	depth = d;
}

/** \brief Get this gate's depth. 
 *  \return An integer that gives this gate's depth. -1 indicates the gate's depth has not been initialized.
 */
int Gate::getDepth() {
	return depth;
}
